...
# 如果路径不对，手动改变工作目录
cd("f:/qcodes/JuliaOptimizeNpa/juliaPython3")  # 确保切换到正确目录
...
# 读取数据并提取需要的参数
bello_data = read_bello_json("bello3_output.json")  # 假设输出文件为output_results.json
...
for entry in bello_data
    ...
    if abs(A14top - npa_value) < 9e-4
        ...
        push!(valid_results, Dict(
            ...
            "A14top - NPA" => A14top - npa_value, 
            "A14top - max_eigenvalue" => A14top - max_eigenvalue, 
            "max_eigenvalue - NPA" => max_eigenvalue - npa_value
        ))
  # else
